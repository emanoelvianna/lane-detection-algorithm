 ...
 * Comando de execução:
 *
 * export OMP_WAIT_POLICY=PASSIVE && ./run_lane_detection_openmp_v4 <video_file_name> <number_of_threads>
 * ------------------------------------------------------------------------------------------
...
void parallel_processing() 
{
	...
	#pragma omp parallel	
	{
		...
		else if( (omp_get_thread_num() != 0) && (omp_get_thread_num() != 1))
		{
			block_two();
		} 
		else
...
int main(int argc, char* argv[]) 
{
	...
	oVideoWriter.open("result_openmp_v4.avi", CV_FOURCC('P','I','M','1'), 20, frameSize, true); 	
