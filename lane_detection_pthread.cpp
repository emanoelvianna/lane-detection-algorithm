...
void parallel_processing()
{
	...
	threads_stage_two = (pthread_t *) malloc(threads_number * sizeof(pthread_t));

	//cria as threads dos estágios
	pthread_create(&thread_stage_one, NULL, stage_one, (void*) NULL);
	for(int i = 0; i < threads_number; i++)
	{			
		pthread_create(&threads_stage_two[i], NULL, stage_two, (void*) NULL);
	}
	pthread_create(&thread_stage_three, NULL, stage_three, (void*) NULL);

	//sincroniza as threads dos estágios
	pthread_join(thread_stage_one, NULL);
	//printf("STAGE ONE TERMINATED!\n");	
	for(int i = 0; i < threads_number; i++)
	{			
		pthread_join(threads_stage_two[i], NULL);
	}
	//printf("STAGE TWO TERMINATED!\n");
	pthread_join(thread_stage_three, NULL);	
	//printf("STAGE THREE TERMINATED!\n");
...
int main(int argc, char* argv[]) 
{
	...
	oVideoWriter.open("result_pthread.avi", CV_FOURCC('P','I','M','1'), 20, frameSize, true); 	
